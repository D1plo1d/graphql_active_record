class GraphQL::DefinitionHelpers::DefinedByConfig::DefinitionConfig
  def active_record_connection(name, opts = {})
    ...
    opts[:type] ||= -> {"#{singular_name}_type".classify.constantize}
    opts[:query] ||= -> {class_name.constantize.all}
    # TODO: only add fields at the top level
    # Field
    field name.to_s.singularize.to_sym do
      type opts[:type]
      argument :id, types.String
      resolve -> (obj, args, ctx) {
        query = opts[:query].call
        if args["id"].present?
          type, id = CloudNodeIdentification.from_global_id args["id"]
          # # TODO: Reimplement Security
          # if class_name != type.to_s
          #   raise(
          #     "id does not have correct type " +
          #     "(expected: #{class_name}, got: #{type.to_s})"
          #   )
          # end
    ...
    # Connection
    connection(name, -> {opts[:type].call.connection_type}) do
      resolve -> (obj, args, ctx) {
        ap "connection resolve"
        ap name
        # Nested connections/ActiveRecord associations
        if obj.is_a?(ActiveRecord::Base) && obj.respond_to?(name)
          next obj.send name
        # Top level connections
        else
          query = opts[:query].call
          eager_load_args = GraphQL::ActiveRecord.eager_load_args_for(
            ctx.ast_node
          )
          query = query.eager_load *eager_load_args if eager_load_args.present?
          # TODO: Where args
          # query = query.where(
          #   GraphQL::ActiveRecord.where_args_for ctx.ast_node
          # )
          next query
        end
      }
